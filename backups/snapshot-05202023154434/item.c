...
int get_name_attachments_from_post(char* url, char* name, char** attachments, char* access_token) {
	...
	const nx_json* attachments_arr = nx_json_get(nx_json_get(nx_json_get(nx_json_get(response_json, "attachments"), "data"), "subattachments"), "data");
...
int item_main(char** args, int argc) {
	...
	char** test_attachments = new_string_arr(12);
